...
def coin(request, parameter):
    ...
    data = requests.get('https://api.coingecko.com/api/v3/coins/'+parameter+'/market_chart?vs_currency=usd&days=30')
    ...
    if user_data is not None:
        ...
        for nam in names:
            ...
            if i == 1 and vol is not None:
                ...
                data = requests.get('https://api.coingecko.com/api/v3/coins/'+n+'/market_chart?vs_currency=usd&days=30')
                data = data.json()
        ...
        fill = {'form': form, 'data': coins, 'graph1': graph1, 'graph2': graph2, \
            'graph3': graph3, 'graph4': graph4, 'graph5': graph5, 'var': round(VaR * 100, 2),\
             "var_usd": round(VaR * portfolio_value * 100, 2), 'ES': round(ES * 100, 2)}
        return render(request, 'pages/port.html', fill)
    else:
        return render(request, 'pages/port.html', {'form': form})